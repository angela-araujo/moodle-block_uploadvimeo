(function(global) {
    ...
    function sendFile(files, accesstoken, username) {
        ...
        if (!MY_APP.utils.isVideo(file.name)) {
            Swal.fire({
                heightAuto: false,
                text: `Houve um problema com seu arquivo. Talvez ele não seja um arquivo de vídeo ou ele usa um codec que não aceitamos!`,
                icon: 'warning',
            });
            return;
        }
        ...
        Swal.fire({
            ...
            title: "Enviar vídeo",
            html: '<input id="swal-input1" placeholder="Título do vídeo" autofocus class="swal2-input" style="min-width: 100%">' +
